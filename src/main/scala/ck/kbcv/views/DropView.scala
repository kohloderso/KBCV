...
class DropView(context: Context, attrs: AttributeSet, symbolListener: DropSymbolsEditor = null) extends CardView(context, attrs) {
    ...
    this.setCardBackgroundColor(ContextCompat.getColor(context, R.color.colorSecondaryBright))
    this.setOnDragListener(new View.OnDragListener() {
        override def onDrag(v: View, event: DragEvent): Boolean = {
            ...
            action match {
                ...
                case DragEvent.ACTION_DRAG_EXITED => v.setBackgroundColor(ContextCompat.getColor(context, R.color.colorAccent))
                case DragEvent.ACTION_DRAG_ENDED => v.setBackgroundColor(ContextCompat.getColor(context, R.color.colorAccent))
